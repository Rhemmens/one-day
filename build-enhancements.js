...
async function addExerciseFromBuild(){
  ...
  const prescribed_sets=Number(document.getElementById('buildExerciseSets')?.value||0);
  const prescribed_reps=(document.getElementById('buildExerciseReps')?.value||'').trim();
  if(!name){status='Type an exercise first.';render();return}
  if(!Number.isFinite(prescribed_sets)||prescribed_sets<1||!prescribed_reps){status='Enter sets and reps for this client.';render();return}
  let exercise=exercises.find(e=>e.name.trim().toLowerCase()===name.toLowerCase());
  if(!exercise){
    const{data,error}=await sb.from('exercises').insert({coach_id:currentUser.id,name,muscle_group,default_sets:prescribed_sets,default_reps:prescribed_reps}).select().single();
    if(error){status=error.message;render();return}
    exercise=data;
    exercises=[...exercises,exercise].sort((a,b)=>a.name.localeCompare(b.name));
    status=`Created “${name}” and added it to this day.`;
  }else status=`Added “${exercise.name}” with ${prescribed_sets} sets × ${prescribed_reps}.`;
  draftTasks.push({type:'workout',name:exercise.name,exercise_id:exercise.id,prescribed_sets,prescribed_reps,superset_key:'',superset_rest_seconds:null});
  render();
}

buildHTML=function(){
  const draft=draftTasks.map((t,i)=>`<div class="card"><div class="row"><div class="grow"><b>${esc(t.name)}</b><p class="small">${t.type==='habit'?'Habit':`${t.prescribed_sets||''} sets × ${esc(t.prescribed_reps||'')} reps`}</p></div><button class="btn" onclick="removeDraft(${i})">Remove</button></div>${t.type==='workout'?`<div class="stack" style="margin-top:10px"><p class="small">Adjust this client’s prescription anytime before saving.</p><div class="row"><input type="number" min="1" value="${t.prescribed_sets||''}" onchange="updateDraft(${i},'prescribed_sets',this.value)" placeholder="Sets"><input value="${esc(t.prescribed_reps||'')}" onchange="updateDraft(${i},'prescribed_reps',this.value)" placeholder="Reps e.g. 8-10"></div><div class="row"><input value="${esc(t.superset_key||'')}" onchange="updateDraft(${i},'superset_key',this.value)" placeholder="Superset group e.g. A"><input type="number" min="0" value="${t.superset_rest_seconds??''}" onchange="updateDraft(${i},'superset_rest_seconds',this.value)" placeholder="Rest sec"></div></div>`:''}</div>`).join('');
  const exerciseOptions=exercises.map(e=>`<option value="${esc(e.name)}">${esc(e.muscle_group)}</option>`).join('');
  ...
  <div class="card"><p class="eyebrow">ADD EXERCISE</p><b>Type or choose an exercise</b><p class="small">Choose the exercise, then enter sets and reps specifically for this client. Nothing is preset.</p><div class="stack" style="margin-top:10px"><input id="buildExerciseName" list="exerciseOptions" placeholder="e.g. Goblet Squat" autocomplete="off"><datalist id="exerciseOptions">${exerciseOptions}</datalist><div class="row"><input id="buildExerciseSets" type="number" min="1" placeholder="Sets"><input id="buildExerciseReps" placeholder="Reps e.g. 8-10"></div><input id="buildExerciseMuscle" value="General" placeholder="Muscle group"><button class="btn primary" onclick="addExerciseFromBuild()">+ Add exercise to day</button></div></div>
